...
from discrete_optimization.generic_tools.hub_solver.optal.generic_optal import OptalSolver
from discrete_optimization.generic_tools.hub_solver.optal.model_collections import problem_to_script_path, DoProblemEnum
...
def tsp_to_dict(problem: TspProblem,
                scaling: float = 100):
    """
    ...
    if hasattr(problem, "distance_matrix") and problem.distance_matrix is not None:
        dist_matrix = np.asarray(scaling*problem.distance_matrix,
                                 dtype=int)
    else:
        dist_matrix = np.asarray(scaling*build_matrice_distance(
            problem.node_count, problem.evaluate_function_indexes
        ), dtype=int)
    return {"node_count": problem.node_count,
...
class OptalTspSolver(OptalSolver):
    ...
    def retrieve_current_solution(self, dict_results: dict) -> TspSolution:
        ...
        return TspSolution(problem=self.problem,
