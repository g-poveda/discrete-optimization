...
from discrete_optimization.gpdp.solvers.ortools_routing import ParametersCost, OrtoolsGpdpSolver
...
class OrtoolsTspTwSolver(SolverDO):
    ...
    def init_model(self, time_limit=10,
                   scaling: float = 100,
                   **kwargs: Any) -> None:
        gpdp_problem = ProxyClass.from_tsptw_to_gpdp(self.problem, True)
        solver = OrtoolsGpdpSolver(problem=gpdp_problem,
                                   factor_multiplier_time=scaling)
        solver.init_model(
            ...
            parameters_cost=[ParametersCost(dimension_name="Time",
                                            global_span=True)],
        )
        self.solver = solver
